#' =============================================================================
#' POOLED RATIO MODEL WITH AR FILTER - OPTIMIZED
#' =============================================================================
#'
...
#' - ALL RATIO features (from Phase 2)
...
MODEL_NAME <- "pooled_ratio_with_ar_optimized"
FEATURE_TYPE <- "ratio"
FILTER_VARIANT <- "WITH_AR_FILTER"

# Class weighting - similar to XGBoost scale_pos_weight
# Weight crisis events higher to handle imbalance
CLASS_WEIGHT_RATIO <- 10  # Similar to 10:1 FN:FP cost ratio in XGBoost
...
cat("=" , rep("=", 78), "\n", sep="")
cat("PHASE 3: POOLED RATIO MODEL - OPTIMIZED\n")
cat("=" , rep("=", 78), "\n", sep="")
cat("\nModel:", MODEL_NAME, "\n")
...
cat("\n", rep("-", 40), "\n", sep="")
cat("Loading ratio features...\n")

features_file <- file.path(PHASE2_DIR, "ratio_features_h8.csv")

if (!file.exists(features_file)) {
  stop("Ratio features file not found. Please run Phase 2 first.")
}
...
cat("   Found", length(ratio_cols), "ratio columns\n")
...
# =============================================================================
# CREATE CLASS WEIGHTS (OPTIMIZATION 2)
# =============================================================================

cat("\n", rep("-", 40), "\n", sep="")
cat("Creating class weights for imbalance handling...\n")

# Weight crisis events by CLASS_WEIGHT_RATIO, non-crisis by 1
df <- df %>%
  mutate(
...
cat("\n", rep("-", 40), "\n", sep="")
cat("Using all ratio features (no feature selection)...\n")

# Use all ratio features
selected_features <- ratio_cols
...
summary_list <- list(
  ...
  n_features_original = length(ratio_cols),
...
cat("   1. All Features Used (", length(selected_features), " features)\n", sep="")
