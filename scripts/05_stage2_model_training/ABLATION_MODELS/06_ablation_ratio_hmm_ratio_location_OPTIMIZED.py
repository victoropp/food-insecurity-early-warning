"""
Stage 2 Ablation Model 7: XGBoost with Ratio + HMM Ratio + Location Features - OPTIMIZED
========================================================================================
Train XGBoost using ratio features (9) + HMM ratio features (3) + location encoding (3) = 15 total features
...
This tests if HMM regime transition features add value to ratio baseline.
...
OUTPUT_DIR = STAGE2_MODELS_DIR / "ablation" / "ratio_hmm_ratio_location_optimized"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

print("=" * 80)
print("ABLATION MODEL 7 - OPTIMIZED: RATIO + HMM RATIO + LOCATION (15 FEATURES)")
print("=" * 80)
print(f"\nModel: ablation_ratio_hmm_ratio_location_optimized")
...
# ABLATION 7: Ratio + HMM Ratio + Location (NO zscore, NO DMD)
feature_cols = ratio_cols + hmm_ratio_cols + location_cols
...
print()
print("-" * 80)
print("STEP 4b: Improved Imputation for HMM Features")
print("-" * 80)

# Country-median imputation for HMM features (instead of fillna(0))
for col in hmm_ratio_cols:
...
total_combinations = 2*3*3*3*2*2*3*2*2
...
for fold in range(N_FOLDS):
    ...
    fold_predictions['model'] = 'ablation_ratio_hmm_ratio_location_optimized'
...
predictions_file = OUTPUT_DIR / "ablation_ratio_hmm_ratio_location_optimized_predictions.csv"
predictions_df.to_csv(predictions_file, index=False)
print(f"[OK] Predictions: {predictions_file.name}")

# CV results
cv_file = OUTPUT_DIR / "ablation_ratio_hmm_ratio_location_optimized_cv_results.csv"
...
summary_file = OUTPUT_DIR / "ablation_ratio_hmm_ratio_location_optimized_summary.json"
...
print(f"\nOUTPUT FILES:")
print(f"  - ablation_ratio_hmm_ratio_location_optimized_predictions.csv")
print(f"  - ablation_ratio_hmm_ratio_location_optimized_cv_results.csv")
print(f"  - country_metrics.csv")
print(f"  - feature_importance.csv")
print(f"  - ablation_ratio_hmm_ratio_location_optimized_summary.json")
