"""
Stage 2 Ablation Model 2: XGBoost with Ratio + Zscore + DMD + Location (NO HMM) - OPTIMIZED
============================================================================================
Train XGBoost using ratio + zscore + DMD + location features (NO HMM)
...
This tests the contribution of DMD features WITHOUT HMM features.
...
OUTPUT_DIR = STAGE2_MODELS_DIR / "ablation" / "ratio_zscore_dmd_location_optimized"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

print("=" * 80)
print("ABLATION MODEL 2 - OPTIMIZED: RATIO + ZSCORE + DMD + LOCATION (NO HMM)")
print("=" * 80)
print(f"\nModel: ablation_ratio_zscore_dmd_location_optimized")
...
# =============================================================================
# PREPARE FEATURES - RATIO + ZSCORE + DMD + LOCATION (NO HMM)
# =============================================================================

print()
print("-" * 80)
print("STEP 4: Preparing Features (ABLATION: NO HMM)")
print("-" * 80)
...
# ABLATION 2: Ratio + Zscore + DMD + Location (NO HMM)
feature_cols = ratio_cols + zscore_cols + dmd_cols + location_cols
...
print(f"DMD features: {len([f for f in feature_cols if f in dmd_cols])}")
print(f"Location features: {len(location_cols)}")
print(f"Total features: {len(feature_cols)}")
print(f"EXCLUDED: HMM features")
...
for fold in range(N_FOLDS):
    ...
    model_file = OUTPUT_DIR / f"ablation_dmd_fold_{fold}.pkl"
    ...
    fold_predictions['model'] = 'ablation_ratio_zscore_dmd_location_optimized'
...
predictions_file = OUTPUT_DIR / "ablation_dmd_optimized_predictions.csv"
predictions_df.to_csv(predictions_file, index=False)
print(f"[OK] Predictions: {predictions_file.name}")

cv_file = OUTPUT_DIR / "ablation_dmd_optimized_cv_results.csv"
...
summary = {
    'model': 'ablation_ratio_zscore_dmd_location_optimized',
    'model_type': 'ABLATION - Ratio + Zscore + DMD + Location (NO HMM)',
    'description': 'Ablation Model 2: Ratio + Zscore + DMD + Location (NO HMM) with GridSearchCV',
    ...
    'features': {
        ...
        'hmm': 0,
        'dmd': len([f for f in feature_cols if f in dmd_cols]),
        'location': len([f for f in feature_cols if f in location_cols]),
        'excluded': 'HMM features'
    },
...
summary_file = OUTPUT_DIR / "ablation_dmd_optimized_summary.json"
...
print()
print("=" * 80)
print("ABLATION MODEL 2 - OPTIMIZED COMPLETE (NO HMM)")
print("=" * 80)
print(f"\nFeatures: {len(feature_cols)} (Ratio + Zscore + DMD + Location)")
print(f"Excluded: HMM features")
...
print(f"\nOUTPUT FILES:")
print(f"  - ablation_dmd_optimized_predictions.csv")
print(f"  - ablation_dmd_optimized_cv_results.csv")
print(f"  - country_metrics.csv")
print(f"  - feature_importance.csv")
print(f"  - ablation_dmd_optimized_summary.json")
