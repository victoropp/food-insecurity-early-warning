"""
Stage 2 Ablation Model 8: XGBoost with Zscore + HMM Zscore + Location Features - OPTIMIZED
==================================================================================
Train XGBoost using zscore features (9) + HMM zscore features (3) + location encoding (2) = 15 total features
...
This isolates the predictive power of zscore features alone without ratio, HMM, or DMD.
...
OUTPUT_DIR = STAGE2_MODELS_DIR / "ablation" / "zscore_hmm_zscore_location_optimized"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

print("=" * 80)
print("ABLATION MODEL 8 - OPTIMIZED: ZSCORE + LOCATION (11 FEATURES)")
print("=" * 80)
print(f"\nModel: ablation_zscore_hmm_zscore_location_optimized")
...
# =============================================================================
# PREPARE FEATURES - ZSCORE + LOCATION ONLY
# =============================================================================

print()
print("-" * 80)
print("STEP 4: Preparing Features (ABLATION: Zscore + Location ONLY)")
print("-" * 80)
...
# ONLY zscore features (not ratio)
zscore_cols = [f'{cat}_zscore' for cat in macro_categories]

# HMM zscore features (3 features)
hmm_zscore_cols = [
...
# ABLATION 8: Zscore + HMM Zscore + Location (NO ratio, NO DMD)
feature_cols = zscore_cols + hmm_zscore_cols + location_cols
...
print(f"HMM Zscore features: {len([f for f in feature_cols if f in hmm_zscore_cols])}")
print(f"Location features: {len(location_cols)}")
print(f"Total features: {len(feature_cols)}")
print(f"EXCLUDED: ratio, DMD")
...
for fold in range(N_FOLDS):
    ...
    # Save model
    model_file = OUTPUT_DIR / f"ablation_zscore_location_fold_{fold}.pkl"
    ...
    fold_predictions['model'] = 'ablation_zscore_hmm_zscore_location_optimized'
...
predictions_file = OUTPUT_DIR / "ablation_zscore_hmm_zscore_location_optimized_predictions.csv"
predictions_df.to_csv(predictions_file, index=False)
print(f"[OK] Predictions: {predictions_file.name}")

# CV results
cv_file = OUTPUT_DIR / "ablation_zscore_hmm_zscore_location_optimized_cv_results.csv"
...
summary = {
    'model': 'ablation_zscore_hmm_zscore_location_optimized',
    'model_type': 'ABLATION - Zscore + Location ONLY',
    'description': 'Ablation Model 4: Zscore + Location ONLY with GridSearchCV optimization',
...
summary_file = OUTPUT_DIR / "ablation_zscore_hmm_zscore_location_optimized_summary.json"
...
print()
print("=" * 80)
print("ABLATION MODEL 8 - OPTIMIZED COMPLETE")
print("=" * 80)
print(f"\nFeatures: {len(feature_cols)} (Zscore + Location ONLY)")
print(f"Excluded: ratio, HMM, DMD")
...
print(f"\nOUTPUT FILES:")
print(f"  - ablation_zscore_hmm_zscore_location_optimized_predictions.csv")
print(f"  - ablation_zscore_hmm_zscore_location_optimized_cv_results.csv")
print(f"  - country_metrics.csv")
print(f"  - feature_importance.csv")
print(f"  - ablation_zscore_hmm_zscore_location_optimized_summary.json")
