"""
Stage 2 Ablation Model 9: XGBoost with Ratio + HMM Ratio + Location Features - OPTIMIZED
========================================================================================
Train XGBoost using ratio features (9) + HMM ratio features (3) + DMD ratio features (4) + location encoding (3) = 19 total features
...
- IMPROVED: Country-median imputation for HMM features

This tests if HMM + DMD features add value to ratio baseline.
...
OUTPUT_DIR = STAGE2_MODELS_DIR / "ablation" / "ratio_hmm_dmd_location_optimized"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

print("=" * 80)
print("ABLATION MODEL 9 - OPTIMIZED: RATIO + HMM + DMD + LOCATION (19 FEATURES)")
print("=" * 80)
print(f"\nModel: ablation_ratio_hmm_dmd_location_optimized")
print(f"Filter: WITH_AR_FILTER (IPC <= 2 AND AR == 0)")
print(f"Enhancement: GridSearchCV with Stratified Spatial CV + Country-Median HMM Imputation")
...
# =============================================================================
# PREPARE FEATURES - RATIO + LOCATION ONLY
# =============================================================================

print()
print("-" * 80)
print("STEP 4: Preparing Features (ABLATION: Ratio + Location ONLY)")
print("-" * 80)

# 9 macrocategory features
macro_categories = ['conflict', 'displacement', 'economic', 'food_security', 'governance',
                    'health', 'humanitarian', 'other', 'weather']

# ONLY ratio features (not zscore)
ratio_cols = [f'{cat}_ratio' for cat in macro_categories]

# HMM ratio features (3 features)
hmm_ratio_cols = [
    'hmm_ratio_crisis_prob',
    'hmm_ratio_transition_risk',
    'hmm_ratio_entropy'
]

# DMD ratio features (4 features)
dmd_ratio_cols = [
...
# ABLATION 9: Ratio + HMM Ratio + DMD Ratio + Location (NO zscore)
feature_cols = ratio_cols + hmm_ratio_cols + dmd_ratio_cols + location_cols
...
print(f"HMM Ratio features: {len([f for f in feature_cols if f in hmm_ratio_cols])}")
print(f"DMD Ratio features: {len([f for f in feature_cols if f in dmd_ratio_cols])}")
print(f"Location features: {len(location_cols)}")
print(f"Total features: {len(feature_cols)}")
print(f"EXCLUDED: zscore")
...
print()
print("-" * 80)
print("STEP 4b: Improved Imputation for HMM and DMD Features")
print("-" * 80)

# Country-median imputation for HMM and DMD features (instead of fillna(0))
for col in hmm_ratio_cols + dmd_ratio_cols:
...
param_grid = {
    'n_estimators': [100, 200, 300],           # 3 options (expanded for complex feature set)
...
for fold in range(N_FOLDS):
    ...
    # Save model
    model_file = OUTPUT_DIR / f"ablation_ratio_hmm_ratio_location_fold_{fold}.pkl"
    ...
    fold_predictions['model'] = 'ablation_ratio_hmm_dmd_location_optimized'
...
predictions_file = OUTPUT_DIR / "ablation_ratio_hmm_dmd_location_optimized_predictions.csv"
predictions_df.to_csv(predictions_file, index=False)
print(f"[OK] Predictions: {predictions_file.name}")

# CV results
cv_file = OUTPUT_DIR / "ablation_ratio_hmm_dmd_location_optimized_cv_results.csv"
...
summary = {
    'model': 'ablation_ratio_hmm_dmd_location_optimized',
    'model_type': 'ABLATION - Ratio + Location ONLY',
    'description': 'Ablation Model 7: Ratio + HMM Ratio + Location with GridSearchCV optimization',
    ...
    'features': {
        ...
        'excluded': 'zscore, DMD', 'hmm_ratio': len([f for f in feature_cols if f in hmm_ratio_cols])
    },
...
summary_file = OUTPUT_DIR / "ablation_ratio_hmm_dmd_location_optimized_summary.json"
...
print()
print("=" * 80)
print("ABLATION MODEL 1 - OPTIMIZED COMPLETE")
print("=" * 80)
print(f"\nFeatures: {len(feature_cols)} (Ratio + Location ONLY)")
print(f"Excluded: zscore, HMM, DMD")
...
print(f"\nOUTPUT FILES:")
print(f"  - ablation_ratio_hmm_dmd_location_optimized_predictions.csv")
print(f"  - ablation_ratio_hmm_dmd_location_optimized_cv_results.csv")
print(f"  - country_metrics.csv")
print(f"  - feature_importance.csv")
print(f"  - ablation_ratio_hmm_dmd_location_optimized_summary.json")
