"""
XGBoost Pipeline - Basic Model WITH_AR_FILTER - OPTIMIZED
==========================================================
...
- Same methodology as advanced optimized model for fair comparison
- Optimizes for recall (catching crises is priority)

FEATURES (BASIC - no HMM/DMD):
...
INPUT_FILE = STAGE2_FEATURES_DIR / "phase3_combined" / "combined_basic_features_h8.csv"
OUTPUT_DIR = STAGE2_MODELS_DIR / "xgboost" / "basic_with_ar_optimized"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

print("=" * 80)
print("XGBOOST BASIC MODEL - OPTIMIZED WITH HYPERPARAMETER TUNING")
print("=" * 80)
print(f"\nModel: xgboost_basic_optimized")
...
# =============================================================================
# PREPARE FEATURES (BASIC - NO HMM/DMD)
# =============================================================================

print()
print("-" * 80)
print("STEP 5: Preparing Features (BASIC - No HMM/DMD)")
print("-" * 80)
...
# BASIC features only - NO HMM/DMD
feature_cols = ratio_cols + zscore_cols + location_cols
...
print("\nNOTE: This is the BASIC model - NO HMM or DMD features")
...
# =============================================================================
# HYPERPARAMETER OPTIMIZATION WITH SPATIAL CV
# =============================================================================

X = df_filtered[feature_cols].fillna(0).values  # Convert to numpy array for indexing
...
# =============================================================================
# STEP 6: HYPERPARAMETER OPTIMIZATION WITH GRIDSEARCHCV
# =============================================================================
print()
print("-" * 80)
print("STEP 6: Hyperparameter Optimization with GridSearchCV")
print("-" * 80)
print(f"\nUsing all {len(feature_cols)} features for model training...")

scale_pos_weight = n_negative / n_positive

# Define hyperparameter grid - same as ablation models for fair comparison
...
for fold in range(N_FOLDS):
    ...
    # Save fold model
    model_file = OUTPUT_DIR / f"xgboost_basic_optimized_fold_{fold}.pkl"
    ...
    fold_predictions['model'] = 'xgboost_basic_optimized'
...
# Save final model
final_model_file = OUTPUT_DIR / "xgboost_basic_optimized_final.pkl"
...
predictions_file = OUTPUT_DIR / "xgboost_basic_optimized_predictions.csv"
predictions_df.to_csv(predictions_file, index=False)
print(f"Predictions saved: {predictions_file.name} ({len(predictions_df):,} rows)")

# CV results
cv_file = OUTPUT_DIR / "xgboost_basic_optimized_cv_results.csv"
...
summary = {
    'model': 'xgboost_basic_optimized',
    'model_type': 'BASIC (No HMM/DMD) - All Features',
    ...
    'features': {
        ...
        'ratio_features': len([f for f in feature_cols if f in ratio_cols]),
        'zscore_features': len([f for f in feature_cols if f in zscore_cols]),
        'location_features': len([f for f in feature_cols if f in location_cols])
    },
    ...
    'features': {
        ...
        'hmm': 0,  # BASIC model has no HMM
        'dmd': 0,  # BASIC model has no DMD
...
summary_file = OUTPUT_DIR / "xgboost_basic_optimized_summary.json"
...
print()
print("=" * 80)
print("XGBOOST BASIC OPTIMIZED TRAINING COMPLETE")
print("=" * 80)
print()
print("FEATURES:")
print(f"  Total features: {len(feature_cols)}")
print(f"  - Ratio: {len([f for f in feature_cols if f in ratio_cols])}")
print(f"  - Zscore: {len([f for f in feature_cols if f in zscore_cols])}")
...
print()
print("OUTPUT FILES:")
print(f"  - xgboost_basic_optimized_predictions.csv")
print(f"  - xgboost_basic_optimized_cv_results.csv")
print(f"  - country_metrics.csv")
print(f"  - feature_importance.csv")
print(f"  - grid_search_results.csv")
print(f"  - xgboost_basic_optimized_final.pkl")
print()
