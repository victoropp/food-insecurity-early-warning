"""
Stage 3: Cascade Ensemble with BEST ABLATION MODEL
====================================================
Cascade ensemble using best ablation model (Ratio + Location).

This script creates the cascade ensemble with:
- Best Ablation Model: Ratio + Location features
...
3. If AR = 0: Use ablation model's binary prediction
   - If ablation model = 1: Override to 1 (model detected crisis)
   - If ablation model = 0: Keep as 0 (both agree: no crisis)
...
OUTPUT_DIR = RESULTS_DIR / 'cascade_ablation_best'
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

FIGURES_DIR = BASE_DIR / 'FIGURES' / 'cascade_ablation_best'
FIGURES_DIR.mkdir(parents=True, exist_ok=True)

print("=" * 80)
print("CASCADE ENSEMBLE - BEST ABLATION MODEL")
print("=" * 80)
print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print()
print("Strategy: AR Baseline + Best Ablation Model (Ratio + Location) Override")
print("Logic: Simple binary override (AR=0 cases use ablation model prediction)")
print()
...
# Stage 2: Best Ablation Model (Ratio + Location)
stage2_file = STAGE2_MODELS_DIR / 'ablation' / 'ratio_location_optimized' / 'ablation_ratio_location_optimized_predictions.csv'
print(f"\nLoading Stage 2 (Best Ablation Model - Ratio + Location): {stage2_file.name}")
...
summary = {
    'model': 'Cascade Ensemble - Best Ablation Model',
    'strategy': 'AR Baseline + Best Ablation Model (Ratio+Location) with Simple Binary Override',
...
print()
print("=" * 80)
print("CASCADE ENSEMBLE WITH BEST ABLATION MODEL - COMPLETE")
print("=" * 80)
print()
print("STRATEGY:")
print("  - AR Baseline as primary predictor (using optimal threshold)")
print("  - Stage 2 (Best Ablation Model: Ratio + Location) refines AR=0 cases")
